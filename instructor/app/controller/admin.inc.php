...
if($_SESSION['mydata']->isAdmin){
  if (isset($_GET['suspendStudent'])){
    ...
    header('Location: ' . $_SERVER['HTTP_REFERER']);
  } elseif (isset($_GET['removeInstructor'])){ // New condition for removing an instructor
    $_admin = new admin();
    $_admin->removeInstructor($_GET['removeInstructor']);
    header('Location: ' . $_SERVER['HTTP_REFERER']);
    
  } elseif (isset($_GET['createInvites'])){
    $count = !empty($_POST['count']) ? trim($_POST['count']) : null;
    $_admin = new admin();
    $_admin->generateInvitations($count);
    header('Location: ' . $_SERVER['HTTP_REFERER']);
  } elseif (isset($_GET['clearInvites'])){
    $_admin = new admin();
    ...
    $name = ($_POST['studentName']) ? $_POST['studentName'] : null;
