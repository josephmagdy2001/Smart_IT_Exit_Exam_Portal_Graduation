...
$mails = $_mail->getUnsentMails();
          foreach($mails as $m){
            if($m->type == 0){
              ...
              $name = $resetData['name'];
              $subject = 'Reset Your Password -  EXAM SYSTEM';
            ...
            $percent = ($result->TestDegree > 0)?round((($result->FinalGrade / $result->TestDegree) * 100)) : 0;
              $name = 'Student';
              $subject = 'Result Details - EXAM SYSTEM';
              if($percent >= 50){
              $message = 'Passed Your level is intermediated ' . $result->studentName . ', You have passed the Test';
              $header = 'Passed..';
              }
              else if($percent >= 30){
                $message = 'Passed Your level is good ' . $result->studentName . ', You have passed the Test';
                $header = 'Passed..';
                }
                else if($percent = 100){
                  $message = 'Passed Your level is Professional ' . $result->studentName . ', You have passed the Test <br> Your level is Professional';
                  $header = 'Passed..';
                  }
              else{
              $message = 'Unfortunately Try again ' . $result->studentName . ', You did\'t pass the Test';
              $header = 'Failed..';
              }
